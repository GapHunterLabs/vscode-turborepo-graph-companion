 ...
 * a real number of workspace dependency graphs built -- never on install, never on a timer.
...
const KEY_HIT_COUNT = 'turborepoGraphCompanion.review.hitCount';
const KEY_ANSWERED = 'turborepoGraphCompanion.review.answered';
...
/**
 * Call this once per real workspace dependency graphs built. Safe to call multiple times; a
...
function showPrompt(context: vscode.ExtensionContext): void {
  ...
  void vscode.window
    .showInformationMessage(
      `Turborepo Graph Companion: you've used this ${HITS_BEFORE_PROMPT} times -- if it's saved you time, a rating on the Marketplace helps other developers find it.`,
      rateAction,
      dismissAction,
    )
    .then((selection) => {
